...
export default class JsxParser extends Component {
  ...
  parseJSX(rawJSX) {
    const wrappedJsx = `<View>${rawJSX}</View>`;
    let parsed = [];
    try {
  ...
  parseExpression(expression, key) {
    ...
    console.log("EXP TYPE", expression.type);
  ...
  parseElement(element, key) {
    const { bindings = {}, components = {}, callbacks = {} } = this.props;
    const {
      children = [],
      openingElement: { attributes, name: { name } }
    } = element;

    if (/^(html|head|body)$/i.test(name))
      return children.map(c => this.parseElement(c));

    if (this.blacklistedTags.indexOf(name.trim().toLowerCase()) !== -1)
      return undefined;
    let parsedChildren;
    if (components[name] || canHaveChildren(name)) {
      console.log("CAN HAVE CHILDREN");
    ...
    attributes.forEach(expr => {
      ...
      const matches = this.blacklistedAttrs.filter(re =>
        re.test(attributeName)
      );
      console.log("ATTR NAME", attributeName, "KEY", key);
      if (matches.length === 0) {
        attrs[attributeName] = value;
      } else {
        console.log("MATCHES BLACKLIST", callbacks, "VALUE", value);
        attrs[attributeName] = callbacks[value];
      }
    });
  ...
  render() {
    return <View className="jsx-parser">{this.ParsedChildren}</View>;
  }
}
